...
/* Formatted on 1/3/2023 12:56:03 PM (QP5 v5.294) */
CREATE OR REPLACE FORCE VIEW HCM7_QA.PAY_ELEMENT_LINKS_V
(
   ELEMENT_LINK_ID,
   ELEMENT_LINK_NAME,
   EFFECTIVE_START_DATE,
...
AS
   SELECT PEL.ELEMENT_LINK_ID,
          ...
          PEL.ENTITY_ID,
          PEL.GRADE_ID,
          HR_GENERAL_PKG.DECODE_GRADE (PEL.ENTITY_ID, PEL.GRADE_ID) GRADE,
          PEL.PAYROLL_ID,
          HR_GENERAL_PKG.DECODE_PAYROLL (PEL.ENTITY_ID, PEL.PAYROLL_ID)
             PAYROLL,
          PEL.LOCATION_ID,
          HR_GENERAL_PKG.DECODE_LOCATION (PEL.ENTITY_ID, PEL.LOCATION_ID)
             LOCATION,
          PEL.ORGANIZATION_ID,
          HR_GENERAL_PKG.DECODE_ORGANIZATION (PEL.ENTITY_ID,
                                              PEL.ORGANIZATION_ID)
             ORGANIZATION,
          PEL.POSITION_ID,
          HR_GENERAL_PKG.DECODE_POSITION (PEL.ENTITY_ID,
                                          PEL.POSITION_ID,
                                          PEL.EFFECTIVE_START_DATE)
             POSITION,
          ...
          PCAK.SEGMENT15
             COST_SEGMENT15,
          PCAK_BAL.CONCATENATED_SEGMENTS
             BAL_CON_SEGMENTS,
          PCAK_BAL.SEGMENT1
             BAL_SEGMENT1,
          PCAK_BAL.SEGMENT2
          ...
          PCAK_BAL.SEGMENT15
             BAL_SEGMENT15,
          PEL.LINK_ALL_PAYROLL,
          PEL.STANDARD,
          PEL.PEOPLE_GROUP_ID,
          HPG.CONCATENATED_SEGMENTS
             PEOPLE_GROUP_NAME,
          ...
          PEL.PAY_BASIS_ID
             PAY_BASIS_ID,
          HR_GENERAL_PKG.DECODE_LOOKUP ('EMP_CAT',
                                        PEL.EMPLOYMENT_CATEGORY,
                                        'US')
             EMPLOYMENT_CATEGORY,
          PEL.EMPLOYMENT_CATEGORY
     FROM PAY_ELEMENT_LINKS_T           PEL,
          PAY_COST_ALLOCATION_KEYFLEX_T PCAK,
          HR_PEOPLE_GROUPS_T            HPG,
          PAY_COST_ALLOCATION_KEYFLEX_T PCAK_BAL
    WHERE     PCAK.COST_ALLOCATION_KEYFLEX_ID(+) =
                 PEL.COST_ALLOCATION_KEYFLEX_ID
          AND PCAK_BAL.COST_ALLOCATION_KEYFLEX_ID(+) =
                 PEL.BALANCING_KEYFLEX_ID
          AND HPG.PEOPLE_GROUP_ID(+) = PEL.PEOPLE_GROUP_ID
          AND (   (    PEL.ENTITY_ID IN
                          (SELECT ENTITY_ID
                             FROM GN_USERS_PAYROLLS_T
                            WHERE USER_ID = GN_GLOBAL_PKG.USER_ID)
                   AND GN_GLOBAL_PKG.USER_ID <> 1)
